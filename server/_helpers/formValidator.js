...
module.exports = {
    ...
    validatePasswordChangeForm: (payload) => {
        let error = '';
        let isFormValid = true;
        ...
        if (!payload || typeof payload.oldPassword !== 'string' || payload.oldPassword.trim().length < 3) {
            isFormValid = false;
            error = 'Password must have at least 3 characters.';
        }

        if (!payload || typeof payload.newPassword !== 'string' || payload.newPassword.trim().length < 3) {
            isFormValid = false;
            error = 'Password must have at least 3 characters.';
        }

        if (!payload || payload.newPassword !== payload.confirmedPassword) {
            isFormValid = false;
            error = 'Passwords do not match!';
        }
